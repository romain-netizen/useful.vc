...
inv AS (
 SELECT secondary_review_id,
   ...
   count(*) FILTER (WHERE upper(proposed_action) IN ('REVERIFY','TYPE_UPDATE','TRUE_DUPLICATE_ALIAS','REMAP','KEEP_AND_RETYPE','RETYPE_HISTORICAL') OR (original_relationship_present=true AND upper(proposed_action)='DO_NOT_ADD_AS_EQUITY'))::int true_issues,
   count(*) FILTER (WHERE upper(proposed_action) IN ('DO_NOT_ADD_AS_EQUITY','DO_NOT_ADD_UNTIL_DISCLOSED'))::int non_equity
 FROM public.secondary_investor_reviews GROUP BY secondary_review_id
)
SELECT
...
inv AS (
 SELECT secondary_review_id,
   ...
   count(*) FILTER (WHERE upper(proposed_action) IN ('REVERIFY','TYPE_UPDATE','TRUE_DUPLICATE_ALIAS','REMAP','KEEP_AND_RETYPE','RETYPE_HISTORICAL') OR (original_relationship_present=true AND upper(proposed_action)='DO_NOT_ADD_AS_EQUITY'))::int investor_true_issues,
...
SELECT l.id review_id,c.id company_id,c.name,c.website,c.country,c.category,c.public_state current_primary_state,c.updated_at current_primary_updated_at,
 ...
 l.verified_entity_status,l.entity_status_date,l.status_issue,l.investor_review_summary,l.review_status,l.reviewed_at,l.primary_updated_at_snapshot,
 coalesce(inv.investor_additions,0) investor_additions,coalesce(inv.investor_confirmed,0) investor_confirmed,
 coalesce(inv.investor_true_issues,0) investor_true_issues,coalesce(inv.investor_non_equity,0) investor_non_equity,
...
FROM latest l JOIN public.companies c ON c.id=l.company_id LEFT JOIN inv ON inv.secondary_review_id=l.id LEFT JOIN public.review_feedback rf ON rf.secondary_review_id=l.id
...
async function applyCanonicalDecision(sql,review,decision,finalState,overrides){
  const useQa=decision==='secondary_correct';
  const condition=finalState==='Pending'?(review.key_issue||review.what_first_pass_should_do_next_time||'Pending human adjudication condition'):null;
  await sql`UPDATE public.companies SET public_state=${finalState},publishable=${finalState!=='Excluded'},what_it_needs_to_qualify=${condition},updated_at=now() WHERE id=${review.company_id}`;
  if(useQa){const criteria=await sql`SELECT criterion,proposed_verdict,rationale FROM public.secondary_criterion_reviews WHERE secondary_review_id=${review.id} ORDER BY criterion`;for(const c of criteria){await sql`INSERT INTO public.criterion_reviews(company_id,criterion,verdict,rationale,reviewed_at,methodology_version) VALUES(${review.company_id},${c.criterion},${c.proposed_verdict},${c.rationale},now(),'v5 — human-adjudicated QA') ON CONFLICT(company_id,criterion) DO UPDATE SET verdict=excluded.verdict,rationale=excluded.rationale,reviewed_at=excluded.reviewed_at,methodology_version=excluded.methodology_version`}}
  if(decision==='custom'&&overrides&&typeof overrides==='object'){for(const [k,v] of Object.entries(overrides)){const criterion=Number(k);if(!Number.isInteger(criterion)||criterion<1||criterion>8)continue;const verdict=String(v?.verdict||v||'').toUpperCase();if(!['PASS','FAIL','UNCLEAR'].includes(verdict))continue;const rationale=typeof v==='object'&&v.rationale?String(v.rationale):'Human adjudication override';await sql`INSERT INTO public.criterion_reviews(company_id,criterion,verdict,rationale,reviewed_at,methodology_version) VALUES(${review.company_id},${criterion},${verdict},${rationale},now(),'v5 — human-adjudicated custom') ON CONFLICT(company_id,criterion) DO UPDATE SET verdict=excluded.verdict,rationale=excluded.rationale,reviewed_at=excluded.reviewed_at,methodology_version=excluded.methodology_version`}}
}

async function saveFeedback(req,res,id){const sql=db();const b=await body(req);if(!['primary_correct','secondary_correct','custom'].includes(b.decision))return json(res,400,{error:'invalid_decision'});const [r]=await sql`SELECT id,company_id,original_state,proposed_state,key_issue,what_first_pass_should_do_next_time FROM public.secondary_company_reviews WHERE id=${id}`;if(!r)return json(res,404,{error:'review_not_found'});const finalState=b.decision==='primary_correct'?r.original_state:b.decision==='secondary_correct'?r.proposed_state:b.final_state;if(!['Main','Pending','Excluded'].includes(finalState))return json(res,400,{error:'invalid_final_state'});const note=b.feedback_note?String(b.feedback_note).slice(0,8000):null;const learn=b.use_for_learning!==false;const overridesObj=b.final_criterion_overrides||{};const overrides=JSON.stringify(overridesObj);const [saved]=await sql`INSERT INTO public.review_feedback(company_id,secondary_review_id,decision,final_state,final_criterion_overrides,feedback_note,use_for_learning,learning_status,learning_processed_at) VALUES(${r.company_id},${r.id},${b.decision},${finalState},${overrides}::jsonb,${note},${learn},${learn?'pending':'ignored'},NULL) ON CONFLICT(secondary_review_id) DO UPDATE SET decision=excluded.decision,final_state=excluded.final_state,final_criterion_overrides=excluded.final_criterion_overrides,feedback_note=excluded.feedback_note,use_for_learning=excluded.use_for_learning,learning_status=excluded.learning_status,learning_processed_at=NULL,created_at=now() RETURNING *`;await applyCanonicalDecision(sql,r,b.decision,finalState,overridesObj);await sql`UPDATE public.secondary_company_reviews SET review_status='adjudicated',adjudicated_at=now(),adjudication_note=${note} WHERE id=${id}`;json(res,200,{feedback:saved,canonical_applied:true})}
