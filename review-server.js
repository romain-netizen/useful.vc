...
const investorIssueSql = `upper(proposed_action) IN ('REVERIFY','TYPE_UPDATE','TRUE_DUPLICATE_ALIAS','REMAP','KEEP_AND_RETYPE','RETYPE_HISTORICAL') OR (original_relationship_present=true AND upper(proposed_action)='DO_NOT_ADD_AS_EQUITY')`;

async function summary(res){const sql=db();const [r]=await sql.unsafe(`
...
inv AS (
 SELECT secondary_review_id,
   ...
   count(*) FILTER (WHERE ${investorIssueSql})::int true_issues,
...
async function index(res){const sql=db();const rows=await sql.unsafe(`
WITH latest AS (SELECT DISTINCT ON(company_id) * FROM public.secondary_company_reviews ORDER BY company_id,reviewed_at DESC,id DESC),
inv AS (
 SELECT secondary_review_id,
   ...
   count(*) FILTER (WHERE ${investorIssueSql})::int investor_true_issues,
...
async function applyCanonicalDecision(sql,review,decision,finalState,overrides){
  ...
  if(useQa){
    const criteria=await sql`SELECT criterion,proposed_verdict,rationale FROM public.secondary_criterion_reviews WHERE secondary_review_id=${review.id} ORDER BY criterion`;
  ...
  if(decision==='custom' && overrides && typeof overrides==='object'){
    for(const [k,v] of Object.entries(overrides)){
      const criterion=Number(k);if(!Number.isInteger(criterion)||criterion<1||criterion>8)continue;
      const verdict=String(v?.verdict||v||'').toUpperCase();if(!['PASS','FAIL','UNCLEAR'].includes(verdict))continue;
      const rationale=typeof v==='object'&&v.rationale?String(v.rationale):'Human adjudication override';
