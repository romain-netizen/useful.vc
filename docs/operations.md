...
Repository: `romain-netizen/useful.vc`. `main` is the production branch. Pull requests run syntax, deployment-source, credential-embedding and Cloudflare dry-build checks. No application dataset or secret is committed.
...
The current production version was uploaded by Wrangler from a computer. Cloudflare Workers Builds must be connected to `romain-netizen/useful.vc`, branch `main`, before this path is independent of a local OAuth session. Creating that persistent GitHub OAuth connection requires owner confirmation at the Cloudflare dashboard action itself.
